...
#extract from https://taurus-scada.org/_modules/taurus/qt/qtgui/display/qled.html
import sys
...
#[docs]
    def toggleLedStatus(self):
        ...
        print(self.getLedColor())
        print(self.getLedStatus())
...
def main():
    ...
    layout.setSpacing(2)
    w.setLayout(layout)
    w.setWindowTitle("Feux Clignotants Rouge Vert")
    #for i, color in enumerate(LedColor.keys()):
    #for i, color in enumerate(LedColor.get("RED")):
        #print("i=")
        #print(i)
        #print("color=")
        ##print(color)
        #led = QLed()
        #led.ledColor = LedColor.get(2)
        #led.ledStatus = True
        #layout.addWidget(led, 2, 0)
        #led = QLed()
        #led.ledColor = LedColor.get(2)
        #led.ledStatus = False
        #layout.addWidget(led, 2, 1)

    led = QLed()
    led.ledColor = LedColor.get(1)
    led.setLedStatus(0)
    led.toggleLedStatus()
    led.blinkingInterval = 50400

    layout.addWidget(led, 2)

    led = QLed()
    led.ledColor = LedColor.get(2)
    #led.setLedStatus(1)
    led.toggleLedStatus()
    led.blinkingInterval = 50000
    layout.addWidget(led, 2)

    print(led.getLedColor())
    print(led.getLedStatus())
